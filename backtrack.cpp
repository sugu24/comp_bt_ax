...
int add_bt_hints[5] = { -1, -1, -1, -1, -1 };
int Sudoku::doBacktrack_n(int add, int add_num_of_hint, int bfr_mass, int least_conv) {
    if (add_num_of_hint == 0) {
        doBacktrackAll(false, ZERO, least_conv);
        if (Count > 0 && Count < least_conv) {
            addHints.clear();
            for (int i = 0; i < add; i++) {
                addHints.push_back(add_bt_hints[i]);
            }
            least_conv = Count;
            ConvergeCount17 = Count;
        }
        ...
        for (int mass = bfr_mass + 1; mass < 81; mass++) {
            if (Mass[mass] != 9) 
                continue;
            for (int num = 0; num < 9; num++) {
                if (Skip[mass * 9 + num] || !CandidateNumber[mass * 9 + num])
                    continue;

                discardCandidateNumbers(mass / 9, mass % 9, num);
                add_bt_hints[add_num_of_hint-1] = mass * 9 + num;

                memcpy(Skip, skip, sizeof(skip));

                least_conv = doBacktrack_n(add, add_num_of_hint - 1, mass, least_conv);

                addCandidateNumbers(mass / 9, mass % 9, num);
                add_bt_hints[add_num_of_hint-1] = -1;
            }
        }
        return least_conv;
    }
